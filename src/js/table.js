...
export class RegularTableViewModel {
    ...
    async draw(container_size, view_cache, selected_id, preserve_width, viewport, num_columns) {
        ...
        const {data, row_indices, column_indices, __id_column: id_column} = await view(
            viewport.start_col,
            viewport.start_row,
            viewport.end_col - viewport.start_col,
        ...
        try {
            ...
            while (dcidx < num_visible_columns) {
                if (!data[dcidx]) {
                    ...
                    const new_col = await view(viewport.start_col, viewport.start_row, viewport.end_col - viewport.start_col, viewport.end_row - viewport.start_row);
